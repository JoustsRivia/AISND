...
//   1) 各单一源（shared/dbBase.js、shared/userBase.js、shared/rateLimiter.js）语法有效；
  ...
  for (const [srcName, destName] of SRC_TO_DEST) {
    ...
    if (fs.existsSync(dest)) {
      ...
      if (fs.readFileSync(dest, 'utf8') !== src) {
        console.error(`⚠️ 漂移：${path.relative(ROOT, dest)} 与 shared/${srcName} 不一致，请重跑 node scripts/bundle-db-base.js`);
        bad++;
      }
    } else {
      n++;
...
console.log(`✅ 隔离层单一源校验通过：${n} 个副本均可由 shared/* 一致生成（dbBase.js + userBase.js + rateLimiter.js）`);
