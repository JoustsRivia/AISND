...
// �? 隔离层单一源（可迁移契约的核心）�?
//
// 设计背景：微信云函数「逐函数独立部署」，跨函�? require 共享文件会在运行时失�?
// （部署包只含函数自身目录）。故本文件作为「单一源」，�? scripts/bundle-db-base.js
// 在部�? / 测试前拷贝进每个 cloudfunctions/<fn>/helpers/dbBase.js，使各函数自包含�?
//
// 迁移到自有服务器时：只重写本文件（把 cloud.database() 换成 MySQL/MongoDB 客户�?
// �? collection 工厂），19 �? helpers/db.js 的业务部分零改动�?
//
// 本文件只暴露「与 wx-server-sdk 强耦合」的 4 个原语：cloud / db / _(command) / collection�?
// 以及可直接复用的通用查询原语（regExp / getById / add / update / listBy）�?
// 各函数的领域查询（findUser 等）保留在自�? db.js，便于按需定制排序 / 分页�?
...
// True pagination: iterate a collection in chunks of `size` (WeChat caps 100 docs/get).
// Avoids the silent truncation of listBy's fixed cap. Returns a plain array.
async function listAll(name, filter = {}, opts = {}) {
  ...
  let skip = 0;
  while (true) {
    const res = await col.where(filter).skip(skip).limit(size).get();
    const data = res.data || [];
    if (!data.length) break;
    all = all.concat(data);
    skip += data.length;
    if (data.length < size || all.length >= max) break;
  }
  return all;
}
...
// 组织范围过滤片段（★ 单一源，迁移零改动）：返回可直接合并进列�? where �? orgId 条件�?
// 业务列表统一调用本函数，避免在每个函数里重复子树推导逻辑（「统一注入」数据范围）�?
//   - 全局角色：返�? {}（不过滤，看全量�?
//   - 无可见组织：返回 { orgId: '__unbound__' }（命中空集，杜绝越权可见�?
//   - 其余：返�? { orgId: _.in(subtreeIds) }（按组织子树收窄�?
