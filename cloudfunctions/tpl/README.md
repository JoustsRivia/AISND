# cloudfunctions/tpl —— 云函数脚手架模板（**不可部署**）
...
- `helpers/db.js`、`helpers/user.js` 首行为 `// cloudfunctions/tpl/helpers/...`，明确表示其归属为模板，复制后需改为真实函数路径（已有 CI 脚本 `scripts/helper-comments.js` 校验）。
...
```bash
cp -r cloudfunctions/tpl cloudfunctions/foo
